          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 xl:flex xl:items-center xl:gap-3">
            {steps.map((s, i) => {
              ...
              return (
                <div
                  key={s.title}
                  className="flex items-start gap-3 xl:flex-1 xl:items-center"
                >
                  ...
                  {i < steps.length - 1 && (
                    /* In the two-column layout only the arrow that stays inside a
                       row makes sense; the one-row layout at xl shows all three. */
                    <span
                      className={`ml-auto self-center text-2xl text-gold ${
                        i % 2 === 0 ? "hidden sm:block" : "hidden xl:block"
                      }`}
                    >
                      &#8594;
                    </span>
